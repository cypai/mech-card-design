...
def draw_name(draw_ctx: Drawing, drone: Drone):
    draw_ctx.push()
...
def draw_ability(draw_ctx: Drawing, drone: Drone):
    margin = int(TRACKER_SIZE * 1.2)
    draw_ctx.push()
    draw_ctx.font_size = SMALL_FONT_SIZE
    draw_ctx.fill_color = Color("#000000")
    draw_ctx.text_alignment = "left"
    wrapped_text = wrap_text(draw_ctx, drone.ability, CARD_WIDTH - 2 * margin)
...
def main():
    ...
    print(f"Running drones {args.action}")
